...
# wget https://raw.githubusercontent.com/bitsko/novoconfig/main/novo_node_compile.sh && chmod +x novo_node_compile.sh && ./novo_node_compile.sh
...
if [[ $(uname -m) == "aarch64" ]] || [[ $(uname -m) == "aarch64_be" ]] || \
	...
	if [[ "$novo_OS" == "debian" ]] || [[ "$novo_OS" == "ubuntu" ]]; then
		...
		echo "OS unsupported by this script; ask @bitsko in the telegram chat \
		to add your 64 bit Linux OS"
		script_exit
		unset -f script_exit
		exit 1
	fi
  ...
  script_exit
	...
	ask @bitsko in the telegram chat to add your architecture"
	script_exit
	unset -f script_exit
	exit 1
fi
